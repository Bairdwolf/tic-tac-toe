...
def play_game()
  ...
  until board.winner==1 || board.winner==-1
    ...
    choice=make_choice(board.player)
    board.winner=play_move(player, choice)
  end
...
def play_move(player, choice)
  board.state[choice].value=player
  winner=board.check_squares(choice)
  if winner==0
    game_display.middle_update(board.state[choice])
  end
  winner
end
